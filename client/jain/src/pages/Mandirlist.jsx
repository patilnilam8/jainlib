...
const Mandirlist = () => {
 ...
 formData.images.forEach((file) => {
    ...
    try {
      await axios.post("http://localhost:5001/api/mandir", formDataObj, {
  ...
  editData.images.forEach((file) => {
    ...
    try {
      await axios.put(`http://localhost:5001/api/mandir/${editData._id}`, formDataObj, {
        headers: { "Content-Type": "multipart/form-data" },
      });
      fetchMandir();
      setIsEditing(false);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:5001/api/mandir/${id}`);
      fetchMandir();
    } catch (error) {
